...
- Treat `Shadowrocket.conf` and `Clash_Merge.yaml` as paired configuration sources. When a routing rule, proxy group, default selection, or rule-provider changes in one, make the equivalent change in the other unless the user explicitly requests a client-specific difference.
